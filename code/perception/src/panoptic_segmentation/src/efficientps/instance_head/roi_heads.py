...
from detectron2.modeling.roi_heads import ROI_HEADS_REGISTRY, ROIHeads, \
    BaseMaskRCNNHead, select_foreground_proposals
...
class CustomROIHeads(ROIHeads):
    ...
    mask_pooler (ROIPooler): pooler to extract region features from
        image features.
        The mask head will then take region features to make predictions.
        If None, the mask head will directly take the dict of image features
        defined by `mask_in_features`
    mask_head (nn.Module): transform features to make mask predictions
        keypoint_in_features, keypoint_pooler,
    ...
    def forward(
        ...
        if self.training:
            ...
            # keypoints heads are only
            # applied to the top scoring box detections.
            # Check if all proposal has been removed by the IoU thresh and
            # score thresh
    ...
    def _forward_mask(self, features: Dict[str, torch.Tensor],
        ...
        Args:
            features (dict[str, Tensor]): mapping from feature map names
                to tensor.
                Same as in :meth:`ROIHeads.forward`.
            instances (list[Instances]): the per-image instances to
                train/predict masks.
                In training, they can be the proposals.
                In inference, they can be the boxes predicted
                by R-CNN box head.

        Returns:
            In training, a dict of losses.
            In inference, update `instances` with new fields "pred_masks"
            and return it.
        """
        if self.training:
        ...
        if self.mask_pooler is not None:
            ...
            boxes = [x.proposal_boxes if self.training else x.pred_boxes
                     for x in instances]
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor],
        ...
        Forward logic of the box prediction branch.
            If `self.train_on_pred_boxes is True`,
            the function puts predicted boxes in the `proposal_boxes` field of
            `proposals` argument.
        ...
        box_features = self.box_pooler(features, [x.proposal_boxes
                                                  for x in proposals])
        ...
        if self.training:
            ...
            pred_instances, _ = self.box_predictor.inference(predictions,
                                                             proposals)
            return pred_instances
    ...
    def forward_with_given_boxes(
        ...
        This is useful for downstream tasks where a box is known, but need to
        obtain other attributes (outputs of other heads).
        Test-time augmentation also uses this.

        Args:
            features: same as in `forward()`
            instances (list[Instances]): instances to predict other outputs.
                Expect the keys "pred_boxes" and "pred_classes" to exist.
